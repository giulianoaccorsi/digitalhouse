...
class ViewController: UIViewController {
    ...
    @IBAction func butonAddProduct(_ sender: Any) {
        ...
        let actionDone = UIAlertAction(title: "Adicionar", style: .default) { (addProduto) in
            print("Adicionou")
            let produto = self.mp.createProdut()
            self.mp.addListaShop(product: produto)
...
extension ViewController: UITextFieldDelegate {
    ...
    func textFieldDidEndEditing(_ textField: UITextField) {
        if textField.tag == 0 {
            mp.setNome(nome: textField.text)
        }else {
            mp.setQtd(qtd: Int(textField.text ?? ""))
        }
    }
